...
export function useSignalR() {
  ...
  useEffect(() => {
    ...
    const onAnnounce = (p: AnnounceNextPayload) => {
      setAnnounce(p.CourtId, p)
    }
    conn.on('AnnounceNext', onAnnounce)
    conn.on('announcenext', onAnnounce)
  ...
  }, [url, setStatus, setError, upsert, setScene, setSummary, setAnnounce])
