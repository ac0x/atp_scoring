...
import type { Court, MatchSummary, ScorePayload } from '@/types'
...
export const useScoreStore = create<State>((set) => ({
  ...
  upsert: (p)=>set((st)=>{
    ...
    const ok = !current || new Date(p.ServerTimeUtc).getTime() >= new Date(current.ServerTimeUtc).getTime()
